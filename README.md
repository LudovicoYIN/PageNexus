...
- PDF 分片：Python splitter（优先 PyMuPDF）
...
├── python/          PDF splitter 与 Python 依赖
...
- `Model ID`
- `MinerU API Token`
- `Python Runtime Path`（可选，建议填你打包后的内置 Python）
...
1. 如果是大 PDF，先走 Python splitter（失败时回退 Rust 切块）
