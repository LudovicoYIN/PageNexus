...
import { Activity, ArrowLeft, FolderOpen, Globe, Key, RefreshCcw, Settings, Terminal } from "lucide-react";
    ...
    <div className="flex-1 overflow-y-auto rounded-[2rem] bg-white/40 p-8 backdrop-blur-3xl">
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="hero-card mb-6 rounded-[1.6rem] px-5 py-4">
          ...
          <div className="mt-2 text-sm font-semibold text-slate-600">统一管理模型、解析服务与本地存储路径，保存后会自动重连 Agent。</div>
        </div>
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          >
            <Space direction="vertical" size={12} style={{ width: "100%" }}>
              ...
              <Input
                ...
                onChange={(event) => props.onUpdateSettings((current) => ({ ...current, packy_api_base_url: event.target.value }))}
              ...
              <Input
                ...
                onChange={(event) => props.onUpdateSettings((current) => ({ ...current, packy_model_id: event.target.value }))}
                placeholder="gpt-5.4-low"
              />
              <Typography.Text type="secondary">API Key</Typography.Text>
              <Input.Password
                value={props.settings.packy_api_key}
                onChange={(event) => props.onUpdateSettings((current) => ({ ...current, packy_api_key: event.target.value }))}
                placeholder="sk-..."
              />
            </Space>
          </Card>
          ...
          >
            <Space direction="vertical" size={12} style={{ width: "100%" }}>
              <Typography.Text type="secondary">Secret Key</Typography.Text>
              <Input.Password
                value={props.settings.mineru_api_token}
                onChange={(event) => props.onUpdateSettings((current) => ({ ...current, mineru_api_token: event.target.value }))}
                placeholder="MinerU Secret..."
              />
              <Typography.Text type="secondary">Storage</Typography.Text>
              <Input
                value={props.settings.storage_dir}
                onChange={(event) => props.onUpdateSettings((current) => ({ ...current, storage_dir: event.target.value }))}
        ...
        <Space className="mb-6">
          <Button type="primary" icon={<Settings className="h-4 w-4" />} loading={props.isSavingSettings} onClick={props.onSaveSettings}>
            保存设置
          </Button>
